...
class App(ctk.CTk):
    ...
    def create_actions_tab(self, parent):
        ...
        action_toggles_map = {
            'Movies':       ('MOVIES_ENABLED', 'MOVIES_DIR'), 'TV Shows':     ('TV_SHOWS_ENABLED', 'TV_SHOWS_DIR'),
            'Anime Movies': ('ANIME_MOVIES_ENABLED', 'ANIME_MOVIES_DIR'), 'Anime Series': ('ANIME_SERIES_ENABLED', 'ANIME_SERIES_DIR'),
        }
    ...
    def _create_path_entry_row(self, parent, row, dir_key, label_text, enable_key=None):
        label_widget = ctk.CTkCheckBox(parent, text=label_text, variable=self.enabled_vars[enable_key]) if enable_key else ctk.CTkLabel(parent, text=label_text)
    ...
    def create_settings_tab(self, parent):
        ...
        dir_map = {
            'SOURCE_DIR': ('Source Directory', None), 'MOVIES_DIR': ('Movies Directory', 'MOVIES_ENABLED'),
            'TV_SHOWS_DIR': ('TV Shows Directory', 'TV_SHOWS_ENABLED'), 'ANIME_MOVIES_DIR': ('Anime Movies Directory', 'ANIME_MOVIES_ENABLED'),
            'ANIME_SERIES_DIR': ('Anime Series Directory', 'ANIME_SERIES_ENABLED'),
        }
        ...
        self.toggle_french_dir_visibility(); row_counter += 1

        ctk.CTkLabel(parent, text="Sidecar Extensions").grid(row=row_counter, column=0, padx=5, pady=5, sticky="w")
        self.sidecar_entry = ctk.CTkEntry(parent, placeholder_text=".srt, .nfo, .txt, etc.")
        self.sidecar_entry.grid(row=row_counter, column=1, columnspan=2, padx=5, pady=5, sticky="ew")
        if self.config.SIDECAR_EXTENSIONS: self.sidecar_entry.insert(0, ", ".join(self.config.SIDECAR_EXTENSIONS))
        row_counter += 1

        ctk.CTkLabel(parent, text="Custom Strings to Remove").grid(row=row_counter, column=0, padx=5, pady=5, sticky="w")
        self.custom_strings_entry = ctk.CTkEntry(parent, placeholder_text="FRENCH, VOSTFR, 1080p, etc.")
        self.custom_strings_entry.grid(row=row_counter, column=1, columnspan=2, padx=5, pady=5, sticky="ew")
        if self.config.CUSTOM_STRINGS_TO_REMOVE: self.custom_strings_entry.insert(0, ", ".join(self.config.CUSTOM_STRINGS_TO_REMOVE))
        row_counter += 1

        ctk.CTkLabel(parent, text="OMDb API Key").grid(row=row_counter, column=0, padx=5, pady=5, sticky="w")
        self.api_key_entry = ctk.CTkEntry(parent, placeholder_text="Enter API key")
        self.api_key_entry.grid(row=row_counter, column=1, columnspan=2, padx=5, pady=5, sticky="ew")
        ...
        self.api_key_entry.bind("<Key>", self.on_api_key_type); row_counter += 1

        save_button = ctk.CTkButton(parent, text="Save Settings", command=self.save_settings)
        save_button.grid(row=row_counter, column=1, columnspan=2, padx=5, pady=10, sticky="e")
    ...
    def monitor_active_task(self):
        ...
        if is_running:
            ...
            if is_processing: self.stop_button.configure(state="normal", text="STOP", fg_color="#D32F2F", hover_color="#B71C1C")
